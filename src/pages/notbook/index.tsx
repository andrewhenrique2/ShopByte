...
const NotebooksPage = () => {
  const [notebookProducts, setNotebookProducts] = useState<Product[]>([]);

  useEffect(() => {
    // Filtra os produtos cujo ID começa com "Notebook"
    const filteredNotebooks = products.filter(product =>
      product.id.startsWith("Notebook")
    );
    setNotebookProducts(filteredNotebooks);
  }, []);
  ...
  return (
    <section className="mt-24 mx-auto p-8 bg-bgitens rounded-md below-768:bg-container2 below-768:p-0" style={{ maxWidth: 'calc(100% - 10px)' }}>
      <h1 className="text-center text-2xl font-bold mb-6 text-verdao">Notebooks Disponíveis</h1>
      <div className="grid gap-4 p-4 lg:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-5 6xl:grid-cols-8">
        {notebookProducts.length > 0 ? (
          ...
          <p className="text-center col-span-full text-red-700">Nenhum notebook disponível no momento.</p>
        )}
      </div>
    </section>
  );
