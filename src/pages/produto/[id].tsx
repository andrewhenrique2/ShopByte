...
import { useHandleFavoriteToggle } from '../../utils/handleFavoriteToggle';
...
const ItemDetail = ({
  ...
  const lastInstallmentValue = getLastInstallmentValue(installments);
    ...
    <div className="container mx-auto px-8 pt-5 pb-4 bg-white max-w-screen-2xl rounded-md m-24 below-768:bg-container pt-24 below-768:m-0">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div className="relative h-full w-full flex justify-center items-center">
          {/* Imagem principal */}
          <div className="relative left-0 md:left-24 h-full w-full max-w-[500px] below-1500:max-w-[300px] below-1000:max-w-[200px]">
            <Image
              src={mainImage}
              alt={title}
              layout="fill"
          ...
          <div className="absolute flex flex-col left-[150px] md:items-start md:left-[250px] top-12 below-768:left-[10px]">
            ...
            {memory && (
              <div className="text-white px-2 font-black bg-black rounded-md text-[16px] mb-1 border-solid flex flex-col below-768:text-[12px]">
                Memória{' '}
                <span className='text-pink font-black text-[20px] below-768:text-[12px]'>
                  {memory}
                </span>
              </div>
            )}
            {storage && (
              <div className="text-white px-2 font-black bg-black rounded-md text-[16px] mb-1 border-solid flex flex-col below-768:text-[12px]">
                Armazenamento{' '}
                <span className='text-cian font-black text-[20px] below-768:text-[12px]'>
                  {storage}
                </span>
              </div>
            )}
            ...
            <div className="flex w-full mb-4">
              <div className="flex items-center justify-center border border-0.01 border-black rounded-md rounded-e-none w-full">
                <div className="text-center">
                  <h2 className="text-[#e15219] font-black">
                    {isOnPromotion ? 'FÉRIAS MODO ON' : 'OFERTA ESPECIAL'}
                  </h2>
